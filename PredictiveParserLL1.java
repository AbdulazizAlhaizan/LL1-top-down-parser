...
public class PredictiveParserLL1 {
  ...
  private HashMap<String, String>
      table; // key =  terminal, value = (key = nonT , value= productions )
  ...
  public void InsertProduction(
      String nonTerminal, ArrayList<String> production) {
  ...
  public void addTerminal(String terminal) {
    ...
    set.add(terminal);
    first.put(terminal,
        set); // the first for each nonTerminal = the nonterminal itself
  }
  ...
  public void constructFirstSets() {
    ...
    for (String nonTerminal : nonTerminalsSet) {
      boolean isRooted = true; // if the cutrrent nonT contains another nonT aka
                               // first(S) = first(E)
      while (isRooted) {
        isRooted = false;
        HashSet<String> addrooted =
            new HashSet<>(); //  holds sets of the recurcion nonT
        HashSet<String> removeRooted =
            new HashSet<>(); // holds nonT to be removed
        System.out.println(nonTerminal + ":");
        for (String string : first.get(nonTerminal)) {
          System.out.println(string);
        }
        for (String elemnt : first.get(nonTerminal)) {
          if (nonTerminalsSet.contains(elemnt)
              && !elemnt.equals(nonTerminal)) { // E=E infinet loop
  ...
  public static void main(String[] args) {
    ...
    while (t-- > 0) {
      ...
      System.out.println(
          "if the rule contains an or operator aka'S -> aB | b' please enter 2 not 1!");
      int rulesNumber = in.nextInt();

      while (rulesNumber-- > 0) {
        System.out.println(
            "Enter the right hand side of production rule without spaces");
        String rule = in.next();
